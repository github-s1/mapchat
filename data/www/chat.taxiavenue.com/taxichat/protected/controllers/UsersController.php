...
class UsersController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'type_atributes', 'model_atributes'),
				'users'=>array('*'),
			),
	...
	{	
		...
		{	
			...
			if($user->validate()) {
				...
				if($user->id_type != 1 && $id != 0) {
					DriverService::model()->deleteAll('id_driver = ?' , array($user->id));
					DriverClass::model()->deleteAll('id_driver = ?' , array($user->id));
				}
				if($user->save()) {
					if($id == 0)
						$user->save();
					...
					if(isset($_POST['DriverClass']['id']) && !empty($_POST['DriverClass']['id'])) {
						...
						foreach($_POST['DriverClass']['id'] as $class_id) {
							$class_dr = new DriverClass;
							$class_dr->id_driver = $user->id;
							$class_dr->id_price_class = $class_id;
							$class_dr->save();
						}	
					}
	...
	public function actionType_atributes($id) {		
		...
		if(!empty($_POST['id_type'])) {
			...
			if($_POST['id_type'] == 1) {
				...
				$services_driver = CHtml::listData(DriverService::model()->findAllByAttributes(array('id_driver' => $user->id)), 'id_service', 'id_service');
				
				$price_class_all = CHtml::listData(PriceClass::model()->findAll(), 'id', 'name');
				$price_class_driver = CHtml::listData(DriverClass::model()->findAllByAttributes(array('id_driver' => $user->id)), 'id_price_class', 'id_price_class');
				
				$this->render('type_atributes',array('model'=>$user, 'services_all'=>$services_all, 'services_driver'=>$services_driver, 'price_class_all'=>$price_class_all, 'price_class_driver'=>$price_class_driver, 'type'=>$_POST['id_type'], 'marks_all'=>$marks_all, 'id'=>$id,));
			} else
				$this->render('type_atributes',array('model'=>$user, 'type'=>$_POST['id_type']));
		}	
	}
	...
	public function actionModel_atributes($id) {		
		...
		if(!empty($_POST['id_marka'])) {
			...
			$all_models_mark = CHtml::listData(Models::model()->findAllByAttributes(array('id_marka' => $_POST['id_marka'])), 'id', 'name');
	...
	{	
		...
		if($user->delete()) {
			if($user->id_type == 1) {
				...
				$car->delete();
				//Cars::model()->deleteAll('id = ?' , array($user->id_car));
				DriverService::model()->deleteAll('id_driver = ?' , array($user->id));
				DriverClass::model()->deleteAll('id_driver = ?' , array($user->id));
			}	
		}
	...
	{	
		...
		$dataProvider=Users::model()->findAll($criteria);
		
		//$dataProvider=new CActiveDataProvider('Users');
		
		$this->render('index',array(
			'dataProvider'=>$dataProvider, 'pages' => $pages
		));
	}
