...
class Models extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('marka, name, bodytype', 'required'),
			array('marka, bodytype', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, marka, name, bodytype', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'marka_car' => array(self::BELONGS_TO, 'Marks', 'marka'),
			'bodytype_car' => array(self::BELONGS_TO, 'Bodytypes', 'bodytype'),
		);
	}
