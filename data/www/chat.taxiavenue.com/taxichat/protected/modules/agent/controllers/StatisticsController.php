	...
	*/
...

	{
		...
		return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				...
				'actions'=>array('index', 'drivers', 'get_driver', 'driver_view', 'driver_info', 'payments_history', 'new_payment', 'orderChart'),
...

	{
		...
		if(empty($driver)) {
			...
			if(isset($_POST['filter']) && ($_POST['filter'] != '')){
				...
				$this->redirect(array('/agent/statistics/driver_info/id/'.$id.$parameters)); 
	...
	{	
		...
		if(!empty($_POST)) {
			...
			$this->redirect(array('drivers'.$parameters)); 
        ...
        $drivers_criteria->mergeWith(array(

				'join'=>'INNER JOIN users driver ON driver.id = t.id_user',

				'condition'=>'driver.id_creator = :agent_id',
				'params' => array(':agent_id' => Yii::app()->user->id),
...

	{
		...
		if(isset($_POST['PaymentsHistory'])) {
			...
			$new_payment->attributes = $_POST['PaymentsHistory'];

			$new_payment->id_user = $id;

			if($_POST['flag'] == 1) {

				$new_payment->id_type = 6;

			} else {

				$new_payment->value = -$new_payment->value;

				$new_payment->id_type = 4;

			}

			$new_payment->balance = $driver->balance + $new_payment->value;

			$new_payment->rating = $driver->rating;

			if($new_payment->save()) {

				$driver->password_old = $driver->password;

				$driver->balance = $new_payment->balance;
