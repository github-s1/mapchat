	...
	<div id="tabs-1" class="clearfix ed_driver inv_tab active">
		...
		<?php if(!empty($services_all)) { ?>
			<?php foreach($services_all as $i => $s) { ?>	
				<input type="checkbox"<?php echo(in_array($i, $services_driver)?' checked=""':'');?> name="DriverService[id][<?=$i?>]" id="service<?=$i?>">
	...
	<?php } ?>
	  ...
	  <div class="ed_photos clearfix">
		<fieldset>
			...
			<div>
				<p>Водитель</p>
				<div class="img_file" style="background-image: url(<?php echo(!empty($driver->photo)?Yii::app()->params['siteUrl'].'/images/users_temp/'.$driver->photo:Yii::app()->params['siteUrl'].'/img/ed_photo.png');?>);">
			...
			<div>
				<p>Машина</p>
				<?php for($i = 0; $i < 3; $i++) { ?>
					<div class="img_file" style="background-image: url(<?php echo(!empty($img_fields[$i])?Yii::app()->params['siteUrl'].'/images/cars_temp/'.$img_fields[$i]:Yii::app()->params['siteUrl'].'/img/ed_photo.png');?>);">
					  <div class="img_file_inner">
