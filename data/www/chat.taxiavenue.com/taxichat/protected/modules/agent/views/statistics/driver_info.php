...
<script type="text/javascript">
	$(document).ready(function(){
		$('#autocomplete').autocomplete("<?php echo Yii::app()->request->baseUrl; ?>/agent/statistics/get_driver", {	
			matchContains: false,
			scroll: true,
			highlight: false,
			width: 255,
	...
	function PaymentsHistory() {
		$.ajax({
			url: "<?=Yii::app()->params['siteUrl']?>/agent/statistics/payments_history/id/<?=$id?><?php echo(isset($_GET['date_from']) && $_GET['date_from'] != ''?'/date_from/'.$_GET['date_from']:'');?><?php echo(isset($_GET['date_to']) && $_GET['date_to'] != ''?'/date_to/'.$_GET['date_to']:'');?>",
			type: 'get',
	...
	<h1>Статистика</h1>
	<div class="stats">
		<a href="<?php echo Yii::app()->request->baseUrl; ?>/agent/statistics/index">Статистика</a>
		<a href="<?php echo Yii::app()->request->baseUrl; ?>/agent/statistics/drivers" class="active">Водители</a>
		<div class="stats_inner" id="tabs">
			<?php $form=$this->beginWidget('CActiveForm', array(
				'id'=>'filter-form',
				'enableAjaxValidation'=>false,
				'htmlOptions'=>array('enctype'=>'multipart/form-data'),
				'action'=>Yii::app()->request->baseUrl.'/agent/statistics/driver_view',
			)); ?>
			<label>Водитель:
