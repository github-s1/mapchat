...
class DriversController extends Controller
{
					...
					*/	
					...
					if($car->save()) {
						...
						if($driver->save()) {
							...
							$push = new Push(true, $user_status->ios_id, $user_status->android_id);
							
							$res = $push->setMassage('Ваши данные были успешно промодерированы')->setValue('push_type', 3)->sendPush();
							print_r($push); exit;	
							$this->redirect(array('index'));		
						}
	...
	{	
		...
		if($id == 0) {
			...
			//$driver = $this->loadModel($id);
			$car = Cars::model()->findByPk($driver->id_car);
			$payments_history = PaymentsHistory::model()->findAllByAttributes(array('id_driver' => $driver->id),array('order'=>'id ASC'));
		...
		$this->render('update',array(
			'driver'=>$driver, 'car'=>$car, 'id'=>$id, 'services_all'=>$services_all, 'services_driver'=>$services_driver, 'price_class_all'=>$price_class_all, 'bodytype_all'=>$bodytype_all, 'driver_commissions'=>$driver_commissions, 'new_fine'=>$new_fine, 'payments_history'=>$payments_history, 'reviews_driver'=>$reviews_driver, 'user_status'=>$user_status,
		));
	}
    ...
    {	
		...
		$driver_status->moderation = 1;
		if($driver_status->save()) {	
			$push = new Push(true, $driver_status->ios_id, $driver_status->android_id);
			$res = $push->setMassage('Ваши данные были успешно промодерированы')->setValue('push_type', 3)->sendPush();
			$this->redirect(array('index'));
		}	
    }
	...
	public function actionPush()
	{
		...
		*/
		...
		$push = new Push(true, $driver->ios_id, $driver->android_id);
