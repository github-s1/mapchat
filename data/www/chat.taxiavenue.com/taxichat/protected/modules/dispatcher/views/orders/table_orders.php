	      <table class="orders">
	      	<thead>
                ...
                <tbody id="hotTable">
		     	<tr class="emuHeader">
			     	<td colspan="13">Заказы из приложения</td>
		     	</tr>
		     	<?php if(!empty($orders)){ ?>
				<?php foreach($orders as $i => $or) { ?>
					<tr id="trline_<?=$or->id?>">
						...
						<td>
							<a class="edit_with_bg"  onclick="popup('<?php echo Yii::app()->request->baseUrl; ?>/dispatcher/orders/update/id/<?=$or->id?>', 'Новый заказ', 'pop_chat pop_zone pop_rating'); return false;" title="Редактировать"></a>
						...
						<td>
						   <a class="delete_with_bg"  onclick="popup('<?php echo Yii::app()->request->baseUrl; ?>/dispatcher/orders/closeorder/id/<?=$or->id?>', 'Завершить заказ', 'pop_chat pop_zone pop_rating'); return false;" title="Редактировать"></a>
