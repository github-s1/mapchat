...
/*
	...
	function addPoint() {
		...
		$('#filter').append('<div class="row"><strong>Куда</strong><input type="text" class="geocomplete" name="point_add['+id+'][name]" autocomplete="off" value="" placeholder="Введите адресс" maxlength="255" size="60"><input class="latitude" type="hidden" value="" name="point_add['+id+'][latitude]"><input class="longitude" type="hidden" value="" name="point_add['+id+'][longitude]"><input type="text" name="point_add['+id+'][entrance]" placeholder="Подъезд"><a class="delete" onclick="$(this).parent().remove(); update_route(); return false;" href="javascript: void(0);">удалить</a></div>');
		geolocate();
		return false;
	}
...
<div class="settings_container tarifs">
	...
	<div class="row">
			...
			<?php echo $form->dropDownList($order, 'id_driver',$drivers_all,array('empty' => 'Отсутствует'));?>
			<?php echo $form->error($order,'id_driver'); ?>
		</div>
    <div class="row">
			<?php echo $form->labelEx($order,'Простой'); ?>
			<?php echo $form->textField($order, 'down_time');?>
			<?php echo $form->error($order,'down_time'); ?>
		</div>
	<div class="row">
