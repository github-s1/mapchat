...
class PaymentsHistory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_driver, id_type, value, balance, rating', 'required'),
			array('id_driver, id_type, id_order', 'numerical', 'integerOnly'=>true),
			array('value, balance', 'length', 'max'=>8),
			array('rating', 'length', 'max'=>4),
			array('descr, date_create', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_driver, id_type, value, balance, descr, id_order, date_create, rating', 'safe', 'on'=>'search'),
		);
	}
