...
class UsersTemp extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, id_type, phone, name, surname, email, nickname, balance, photo, id_car, password, rating, dop_info, commission, is_percent, id_price_class, id_driver', 'safe', 'on'=>'search'),
		);
	}
	...
	public function phone_validate($attribute)
	{
		...
		if(!empty($user) && $this->id_driver != $user->id)
			$this->addError($attribute, 'Пользователь c таким телефоном уже существует.');
	}
	
	public function nick_validate($attribute)
	{
		$user = Users::model()->findByAttributes(array('nickname' => $this->$attribute));
		if(!empty($user) && $this->id_driver != $user->id)
			$this->addError($attribute, 'Пользователь c таким ником уже существует.');
	...
	public function rememberPhoto(){
		$this->image_old = $this->photo;
		if(isset($_POST['Users']['photo']))
