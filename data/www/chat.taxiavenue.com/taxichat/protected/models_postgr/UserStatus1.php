...
class UserStatus extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('ios_id, android_id, activate_key', 'length', 'max'=>255),
			array('location_update, status_update, is_activate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_status, lat, lng, location_update, status_update, id_user, moderation, ios_id, android_id, is_activate, activate_key', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('ios_id',$this->ios_id,true);
	...
	{	
		if(!empty($tokin_id)) {
			if($os == 1) {
				$this->ios_id = $tokin_id;
				$this->android_id = NULL;
			} else {
				$this->android_id = $tokin_id;
				$this->ios_id = NULL;
			}	
			$this->save();
		}
