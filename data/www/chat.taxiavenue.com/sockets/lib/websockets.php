            ...
            if (!$user->handshake) {
              ...
              if (($message = $this->deframe($buffer, $user)) !== FALSE) {
                if($user->hasSentClose) {
                  ...
                  $socketMessage = file_get_contents('http://localhost/taxi/taxichat/dispatcher/orders/socket');

                  $this->process($user, 'surprise'); // todo: Re-check this.  Should already be UTF-8.
                }
              } 
              else {
                do {
