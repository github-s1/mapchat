...
class Avatar extends CActiveRecord
{
	...
	protected function beforeSave() {
        ...
        if($image = CUploadedFile::getInstance($this,'big_photo')) {
			...
			@mkdir(Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.'small');
			...
			Yii::app()->ih
			->load($file_src)
			->resize(32, false, true)
			->save(Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.'small'.DIRECTORY_SEPARATOR.$file_name.'.'.$image->getExtensionName());
    ...
    protected function deleteFile($is_delete = false, $photo = null){
		if($is_delete) {
			$documentPath=Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.$this->big_photo;
			$small_documentPath=Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.'small'.DIRECTORY_SEPARATOR.$this->big_photo;	
		} else {
			$documentPath=Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.$photo;
			$small_documentPath=Yii::getPathOfAlias('webroot.img').DIRECTORY_SEPARATOR.$this->directory_name.DIRECTORY_SEPARATOR.'small'.DIRECTORY_SEPARATOR.$photo;
		}
