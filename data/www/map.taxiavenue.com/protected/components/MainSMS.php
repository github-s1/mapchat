...
class MainSMS
{
     ...
     */
    public function __construct($project = false, $key = false, $useSSL = false, $testMode = false)
    {
        if ($project !== false) $this->project = $project;
        if ($key !== false) $this->key = $key;
        if ($useSSL !== false)$this->useSSL = $useSSL;
        if ($testMode !== false)$this->testMode = $testMode;
    }
    ...
    public function messageSend($recipients, $message, $sender, $run_at = null)
    {
        ...
        <SMS>
            ...
            <authentification>
            <username>archic2@mail.ru</username>
            <password>OnlineMap</password>
            </authentification>
            <message>
            <sender>SMS</sender>
            <text>'.$message.' [UTF-8]</text>
            </message>
            <numbers>
